...
export function eloToLambda(homeElo, awayElo, options = {}) {
  // Backtested on 964 WC matches (1930-2022): avg_goals=3.0, divisor=300, goalShareFactor=0.8
  const avgGoals = options.avgGoals || 3.0;
  const divisor = options.divisor || 300;

  const diff = homeElo - awayElo;
  const homeExpWin = 1 / (1 + Math.pow(10, -diff / divisor));

  // Map win expectancy to goal ratio (goalShareFactor=0.8 from backtest)
  const goalShareFactor = options.goalShareFactor || 0.8;
