...
function deriveAsianHandicap(matrix, line) {
  ...
  return {
    line,
    home: round1(homeWin * 100),
    push: round1(push * 100),
    away: round1(awayWin * 100),
  };
}
...
export function computeMatchOdds(lambdaHome, lambdaAway, options = {}) {
  const rho = options.rho ?? -0.04; // Dixon-Coles ρ
  const matrix = buildScoreMatrix(lambdaHome, lambdaAway, rho);
  const result1x2 = derive1X2(matrix);

  // Find best Asian Handicap line
  const ahLines = [-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2];
  const ahResults = ahLines.map(l => deriveAsianHandicap(matrix, l));
  // Pick the line closest to 50-50
  const bestAH = ahResults.reduce((best, cur) =>
    Math.abs(cur.home - 50) < Math.abs(best.home - 50) ? cur : best
  );
  ...
  return {
    ...
    result: result1x2,
    overUnder: {
      "0.5": deriveOverUnder(matrix, 0.5),
      "1.5": deriveOverUnder(matrix, 1.5),
      "2.5": deriveOverUnder(matrix, 2.5),
      "3.5": deriveOverUnder(matrix, 3.5),
    },
    asianHandicap: bestAH,
    allAsianHandicap: ahResults,
...
// Method: λ = attack_strength × opponent_defense_weakness × tournament_avg
export function computeLambda(homeAttack, homeDefense, awayAttack, awayDefense, options = {}) {
  const tournamentAvg = options.avgGoals || 2.6; // WC average total goals per match
  const homeAdvantage = options.homeAdvantage || 1.0; // No home advantage in neutral WC venues
  const halfAvg = tournamentAvg / 2;

  // λ_home = home_attack × away_defense_weakness × avg × home_advantage
  // λ_away = away_attack × home_defense_weakness × avg
  const lambdaHome = homeAttack * awayDefense * halfAvg * homeAdvantage;
  const lambdaAway = awayAttack * homeDefense * halfAvg;
...
// ── ELO to Lambda (fallback when no team stats) ──────────────────────────
// Quick conversion from ELO ratings to expected goals
export function eloToLambda(homeElo, awayElo, options = {}) {
  ...
  // Map win expectancy to goal ratio
  // A team with 60% win expectancy scores ~1.45 goals, concedes ~1.05
  const homeGoalShare = 0.5 + (homeExpWin - 0.5) * 0.6;
