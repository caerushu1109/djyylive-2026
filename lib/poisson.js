...
// ── Compute at specific line ─────────────────────────────────────────────
// Used when bookmaker offers a different line than the model's auto-picked one.
// Recalculates model probability at the bookmaker's line for apples-to-apples comparison.
export function computeAHAtLine(lambdaHome, lambdaAway, line, rho = -0.15) {
  const matrix = buildScoreMatrix(lambdaHome, lambdaAway, rho);
  return deriveAsianHandicap(matrix, line);
}

export function computeOUAtLine(lambdaHome, lambdaAway, line, rho = -0.15) {
  const matrix = buildScoreMatrix(lambdaHome, lambdaAway, rho);
  return deriveOverUnder(matrix, line);
}
