...
const HOST_ADVANTAGE = 1.12;   // +12% λ for host team at home venue
const HOST_OPPONENT_PENALTY = 0.96; // -4% λ for opponent at host venue (crowd pressure)
...
export function hybridLambda(homeAttack, homeDefense, awayAttack, awayDefense, homeElo, awayElo, options = {}) {
  ...
  // ELO-based λ (host advantage already baked into boost, so use raw ELO)
  const eLambda = eloToLambda(homeElo, awayElo, { avgGoals });
