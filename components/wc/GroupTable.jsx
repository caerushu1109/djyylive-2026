    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", marginLeft: "auto" }}>
          {["P","W","D","L","GD","PTS"].map(col => (
            <span key={col} style={{ fontSize: 9, fontWeight: 700, color: "var(--text-muted)", width: 24, textAlign: "center", textTransform: "uppercase" }}>
              {col}
            </span>
          ))}
        </div>
      ...
      {rows.map((row, i) => (
        <div key={row.name} style={{
          ...
          <span style={{ fontSize: 10, color: "var(--text-muted)", width: 14, fontWeight: 700 }}>{row.pos}</span>
          <div style={{ width: 3, height: 24, borderRadius: 3, background: barColor(row.pos), flexShrink: 0 }} />
          ...
          {[row.p, row.w, row.d, row.l].map((v, idx) => (
            <span key={idx} style={{ fontSize: 11, color: "var(--text-dim)", width: 24, textAlign: "center", fontVariantNumeric: "tabular-nums" }}>{v}</span>
          ))}
          <span style={{
            fontSize: 11, color: row.gd > 0 ? "var(--green)" : row.gd < 0 ? "var(--red)" : "var(--text-dim)",
            width: 24, textAlign: "center", fontVariantNumeric: "tabular-nums",
          }}>
            {row.gd > 0 ? `+${row.gd}` : row.gd}
          </span>
          <span style={{ fontSize: 12, fontWeight: 900, color: "var(--text)", width: 24, textAlign: "center", fontVariantNumeric: "tabular-nums" }}>
            {row.pts}
          </span>
        </div>
      ))}
